...
const QuizController = (CUId) => {
  ...
  const securityData = async () => {
    axios
      .all([
        await axios.get("http://localhost:8000/users/" + CUId.CUId),
        await axios.get("http://localhost:8000/exam/exam/" + id.id),
      ])
      .then(
        axios.spread((data, data2) => {
          if (data2.data[0].creatorUserId === CUId.CUId) {
            ...
            alert(
              "You are in preview mode that means your question data will not be saved"
            );
          } else {
            const dummyData = {
              userId: CUId.CUId,
            ...
            axios
              .post("http://localhost:8000/userexams/", dummyData)
              .then((response) => {
