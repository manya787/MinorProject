...
const Dashboard = (CUId) => {
  ...
  const getExamNames = async () => {
    const { data } = await axios.get(`http://localhost:8001/exam/${CUId.CUId}`);
    setExamNameStorage(data);
    setIsLoading(false);
  };

  const deleteExam = (id) => {
    axios.delete(`http://localhost:8001/exam/${id}`).then((response) => {
  ...
  const handleName = (e) => {
    ...
    if (examName == "") {
      ...
      console.log(newExam);
      axios.post("http://localhost:8001/exam/", newExam).then((response) => {
        console.log(response.status);
        console.log(response.data);
      });
      setDummy(dummy + 1);
  ...
  return (
    <>
      ...
      <Container>
        {/* <Wrapper> */}
        <Popup trigger={<CreateButton>Create Exam </CreateButton>} modal nested>
          {(close) => (
            <div
              style={{
                fontSize: "12px",
                  ...
                  <TableCell
                    ...
                    onClick={() => {
                      navigator.clipboard.writeText(
                        "http://localhost:8001/quiz/" + name._id
                      );
                    }}
                  >
                    <span
