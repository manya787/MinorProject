...
const CreateQuiz = () => {
  ...
  const addQuestion = async (e) => {
    ...
    if (correctOption == undefined) {
      ...
      const inputOption = await Promise.all(
        inputFields.map((inputF) => inputF.option)
      );
      ...
      axios
        .post("http://localhost:8000/examquestions/", newQuestion)
        .then((response) => {
  ...
  const handleOptions = ({ data, inputOption }) => {
    ...
    for (let i = 0; i < inputOption.length; i++) {
      ...
      console.log(option);
      axios
        .put("http://localhost:8000/examquestions/" + data, option)
        .then((response) => {
          console.log(response.status);
  ...
  const handleChangeInput = async (id, event) => {
    const newInputFields = await Promise.all(
      inputFields.map((i) => {
        if (id === i.id) {
  ...
  //     const { data } = await axios.get('http://localhost:8000/examquestions/' + id.id);
  //     setExamDatas(data);
  ...
  const getExams = async () => {
    try {
      const { data } = await axios.get(
        "http://localhost:8000/examquestions/" + id.id
      );
  ...
  const deleteQuestion = (propId) => {
    axios
      .delete("http://localhost:8000/examquestions/" + propId)
      .then((response) => {
        console.log(response.status);
                  ...
                  >
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() => deleteQuestion(exam._id)}
                      >
                        Delete
                      </Button>
                    </TableCell>
